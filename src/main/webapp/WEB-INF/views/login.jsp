...
<body>
	<div class="container">
		...
		<form name="loginForm" class="form-signin" action="<c:url value='/j_spring_security_check' />"method="post"
			>
			<h2 class="form-signin-heading">Sistema Saludable</h2>

             <c:if test="${not empty error}">
			<div class="alert alert-danger" role="alert" class="error">${error}</div>
		</c:if>
		<c:if test="${not empty msg}">
			<div class="alert alert-danger" role="alert" class="msg">${msg}</div>
		</c:if>
		
		
			<label for="usuario" class="sr-only">Email address</label> <input
				type="text" name="username" id="imputnick" class="form-control"
